...
(function($){
  ...
  theme.Variants = (function() {
    ...
    Variants.prototype = $.extend({}, Variants.prototype, {
      ...
      _onSelectChange: function() {
        ...
        this._updateRechargeInterval(variant);
      ...
      _updateRechargeInterval: function(variant) {
        var rechargeSelect = $('.rc_select__frequency');
        if(rechargeSelect.length == 0)
        {
          return;
        }

        var variantDays = parseInt(variant.title.split(" ")[0], 10);
        if (variantDays < 10)
        {
          return;
        }

        var shippingInterval = variantDays - 10;
        rechargeSelect.val(shippingInterval);
      },
  ...
  theme.AjaxCart = (function() {
    ...
    var selectors = {
      ...
      itemVariantId: '[data-cart-item-variant-Id]',
    ...
    AjaxCart.prototype = $.extend({}, AjaxCart.prototype, {
      ...
      _createItemList: function(cart) {
        return $.map(cart.items, function(item) {
          ...
          $(selectors.itemVariantId, $item).attr('data-cart-item-variant-id', item.variant_id);
  ...
  $(document).ready(function() {
    ...
    $(document).on('click', '.stamped-main-badge', function(e){
      e.preventDefault();
    ...
    var videoThumbnail = $('.thumbnail-wrapper');
    if(videoThumbnail.length > 0)
    {
      $('.thumbnail-wrapper').slick({
        "slidesToShow": 4,
        "asNavFor": '#Slideshow-video-slider',
        "focusOnSelect": true
      });
    }

    /* setTimeout(function() {
      videoThumbnail.addClass('slick-loaded');
    }, 6000); */
    var storyTestimonialPerson = $('.testimonial-right');
    var storyTestimonialText = $('.testimonial-left');
    if(storyTestimonialPerson.length > 0 && storyTestimonialText.length > 0) 
    {
    ...
    $(document).on('click', '.show-story-form', function(e){
      e.preventDefault();
      $('.form-wrapper').slideToggle();
    });

    $(document).on('click', '.patient-button', function(e){
      e.preventDefault();
      $(this).removeClass('secondary');
      $('.doctor-button').addClass('secondary');
      $(".doctor-testimonial-warpper").addClass('hide');
      $('.patient-testimonial-warpper').removeClass('hide');
      var patientLeft = $('.patient-testimonial-warpper').find('.testimonial-left');
      var patientRight = $('.patient-testimonial-warpper').find('.testimonial-right');
      patientLeft.slick('reinit');
      patientRight.slick('resize');
      patientLeft.slick('slickGoTo', 0);
      patientRight.slick('slickGoTo', 0);
    });

    $(document).on('click', '.doctor-button', function(e){
      e.preventDefault();
      $(this).removeClass('secondary');
      $('.patient-button').addClass('secondary');
      $(".patient-testimonial-warpper").addClass('hide');
      $('.doctor-testimonial-warpper').removeClass('hide');
      var doctorLeft = $('.doctor-testimonial-warpper').find('.testimonial-left');
      var doctorRight = $('.doctor-testimonial-warpper').find('.testimonial-right');
      doctorLeft.slick('resize');
      doctorRight.slick('resize');
      doctorRight.slick('slickGoTo', 0);
      doctorLeft.slick('slickGoTo', 0);
    });

    $(document).on('click', '.mobile-person', function(e) {
  ...
  var player;
  $(document).on('click', '.hero_video-play', function(e) {
    e.preventDefault();
    
    var id = $(this).data('id');
    //player = new YT.Player($(this).data('id'));
    //$('#' + id + '-player').addClass('is-playing');
    $(this).closest('.slideshow__slide').find('.hero__video').addClass('is-playing');
    $(this).closest('.slideshow__slide').find('.hero__video-close').addClass('is-playing');
    //$(this).next('.hero__video-close').addClass('is-playing');
    //player.playVideo();
  });

  $(document).on('click', '.hero__video-close', function(e) {
    $(this).parent().find('.hero__video').removeClass('is-playing');
