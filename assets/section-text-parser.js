function handleSectionText(sectionContainer, userRichText, isCustomize = false) {
	const textContainer = sectionContainer.querySelector('.text-content');
	const desktopDefinition = sectionContainer.querySelector('.definition__desktop');
	let indexerBtn = [];
	...
	formattedText = formattedText.replaceAll(/<\/em>(<a(.*?))<em>(.*?)<\/em>(<\/a>)<em>/g, '$1$3$4');
	formattedText = formattedText.replaceAll(
		/<p>(<a (.*?)>)<em>(.*?)<\/em>(<\/a>)<\/p>/g,
		'<p><em>$1$3$4</em></p>',
	);
	formattedText = formattedText.replaceAll(/(<a (.*?)>)<em>(.*?)<\/em>(<\/a>)/g, '<em>$1$3$4</em>');
	...
	const firstOpenedText = document.querySelector(`.definition__${indexerBtn[0]}`).innerHTML;

	desktopDefinition.innerHTML = firstOpenedText.trim();
	console.log(desktopDefinition.innerHTML);
	desktopDefinition.innerText && desktopDefinition.classList.add('desktop__show');
	const openedMobileDefinition = textContainer.querySelector('.show-definition');
	...
	if (!isCustomize) {
		sectionContainer.addEventListener('click', (e) => {
			...
			const definition = interactiveHTML.querySelector(`.definition__${btnIndex || parentIndex}`);
